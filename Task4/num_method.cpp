 ...
 * based on the index in the 3D array
 */
...
void PdeSolver::communicate(){
	...
	// The local mesh padding will contain the received elements
	for(int req_idx = 0; req_idx < recv_req_size; ++req_idx){
		...
		cur_mesh[global_idx] = received_buf[req_idx].second;
	}
}
...
void PdeSolver::nextIter(){
	...
	communicate();
#ifdef CUDA_ENABLED
	HostMemoryDescr_t host_memory(prev_mesh.getPaddedMesh(), cur_mesh.getPaddedMesh(),
						          next_mesh.getPaddedMesh());
	callMethodIterKernel(&device_memory_, &host_memory, padded_capacity_,
	blk_info_.blk_row_size_, blk_info_.blk_column_size_, blk_info_.blk_z_size_,
	tau_, h_);
#else
	PRAGMA_OMP(_Pragma("omp parallel for"))
	for(int row_idx = blk_info_.blk_row_start_;
