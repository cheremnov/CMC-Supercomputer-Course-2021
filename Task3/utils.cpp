...
bool globalToLocal(const GlobalIndex_t& global_idx, const BlockInfo_t& blk_info,
	...
	local_row_idx = global_idx.row_idx_ - blk_info.blk_row_start_;
	local_column_idx = global_idx.column_idx_ - blk_info.blk_column_start_;
	local_z_idx = global_idx.z_idx_ - blk_info.blk_z_start_;
	if( (local_row_idx < 0 || local_row_idx >= blk_info.blk_row_size_ ) ||
		(local_column_idx < 0 || local_column_idx >= blk_info.blk_column_size_ ) ||
		(local_z_idx < 0 || local_z_idx >= blk_info.blk_z_size_) ){
		return false;
	}
	return true;
}